...
The trading tool runs inside the same Node process but operates independently of the scanner. **Feature-complete as of `f795a30` (2026-07-02)** — see [Trading_Tool_Plan.md](./Trading_Tool_Plan.md) for the full status.

**Flow:**

- **Session** — starts at 9:35 ET, pulls the day's shortlist from the scanner, loads active setups, warms the 6-day historical bar cache (Yahoo primary, Alpaca fallback).
- **Bar poller** — HTTP (60s) or Alpaca WebSocket streams 1-min bars to indicator engines. Router computes multi-day levels (`dailyLevels.js`) on the merged bar buffer at every fire.
- **Router** — evaluates 78 seeded checks (direction-aware, paired variations), asks the Python factor-analysis scorer for a grade (falls back to naive delta or bootstrap B), sizes via three-factor formula (expectancy × winRate × drawdown), applies market gate + kill switch + auto-pause, opens position, fans out to broker profiles that meet the fire's grade (`config.minGrade` gate).
- **brokerSync** — 15s `/v2/orders` poll rewrites `entry_price` with the real fill, closes positions on bracket-leg fills. 60s `/v2/account` poll keeps sizer equity live.
- **Grading engine** — learns from closed `trade_cards`. Naive delta expectancy runs continuously; factor-analysis models (per-setup PCA + Ridge in `src/scoring/setupProcessor.py`) train on demand via the Learning modal or `/setup/train-all`.
- **Journal** — every closed position mirrors to `journal_trades`. Imported CSV/TV-paper history mirrors the other way into `trade_cards`, retroactively evaluated against the check library so grading learns from historical fills too.

**Execution modes** — off / paper / live. Live requires explicit `trading_live_confirmed` acknowledgement in Settings (three-gate safety).

**Alpaca credentials** live only in the Alpaca broker profile (Trading > Brokers).

**Two Python processes under PM2:** `trade-desk` (Node, port 3000) + `scorer` (Python Flask, port 3001; hosts both the scanner scorer and the per-setup factor-analysis scorer).
